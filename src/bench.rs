...
const WORDS: &[&str] = &[
    ...
    "queue", "worker", "shard", "primary", "replica", "commit", "rollback", "index", "scan", "merge",
...
fn worker(dir: &std::path::Path, t: OpLogType, opts: HashSet<OpLogOption>, flush: Duration) -> OpLogWorker {
...
async fn bench_1_producer() {
    ...
    stats("burst 1000 (channel has room), 1 thread", &runs, Some((1000, "call")));
    println!("    allocs/call: {:?}", al.iter().map(|a| *a as f64 / 1000.0).collect::<Vec<_>>());
    ...
    stats("sustained 1M, 1 thread, undefined name", &runs, Some((1_000_000, "call")));
    println!("    dropped so far (channel full): {dropped}");

    // (c) sustained with a defined name (worker does real work + queue cap), 1 thread
    op.def(OpLogDefinition::new("app", dir.to_str().unwrap()).flush_interval(Duration::from_secs(3600)));
    ...
    stats("sustained 1M, 1 thread, defined name", &runs, Some((1_000_000, "call")));
    println!("    dropped during 3M calls: {} ({:.1}%)", d1 - d0, (d1 - d0) as f64 / 3e6 * 100.0);
    ...
    stats("4 threads x 250k, defined name (wall)", &runs, Some((1_000_000, "call")));
    ...
    stats("4 threads x 250k, undefined name (wall)", &runs, Some((1_000_000, "call")));
    ...
    stats("component: 2x to_string (name + 100 B text)", &runs, Some((1_000_000, "call")));
...
async fn bench_2_worker_log() {
    ...
    let cases: Vec<(&str, OpLogType, HashSet<OpLogOption>, usize, usize)> = vec![
        ("PerDay, 100 B", OpLogType::PerDay, HashSet::new(), 100, 50_000),
        ("PerDay, 1 KB", OpLogType::PerDay, HashSet::new(), 1024, 5_000),
        ("PerDay+UseSubDirectories, 100 B", OpLogType::PerDay, HashSet::from([OpLogOption::UseSubDirectories]), 100, 50_000),
        ("PerHour, 100 B", OpLogType::PerHour, HashSet::new(), 100, 50_000),
        ("NoSplit, 100 B", OpLogType::NoSplit, HashSet::new(), 100, 50_000),
        ("PerDay+NoAddDateToLog, 100 B", OpLogType::PerDay, HashSet::from([OpLogOption::NoAddDateToLog]), 100, 50_000),
    ];
    for (label, t, opts, len, n) in cases {
        ...
        for _ in 0..REPS {
            ...
            assert_eq!(w.definitions["app"].files.values().map(|f| f.logs.len()).sum::<usize>(), n);
        }
        stats(&format!("{label} (n={n})"), &runs, Some((n as u64, "entry")));
        println!("    allocs/entry: {:.2}  alloc bytes/entry: {:.0}", al[0], ab[0]);
    }
    ...
    for _ in 0..REPS {
        let ti = Instant::now();
        for s in &texts {
            std::hint::black_box(crate::add_to_log::format_entry(&OpLogType::PerDay, false, &wd, s));
        }
        runs.push(ti.elapsed());
    }
    stats("format_entry (chrono %H:%M:%S.%3f + format!)", &runs, Some((50_000, "entry")));
    println!("    allocs/entry: {:.2}", (allocs().0 - a0) as f64 / (50_000 * REPS) as f64);
    let mut runs = vec![];
    let a0 = allocs().0;
    for _ in 0..REPS {
        let ti = Instant::now();
    ...
    stats("log_name format! + path.to_string()", &runs, Some((50_000, "entry")));
    println!("    allocs/entry: {:.2}", (allocs().0 - a0) as f64 / (50_000 * REPS) as f64);
    let mut runs = vec![];
    let a0 = allocs().0;
    for _ in 0..REPS {
    ...
    stats("file_name key format! (get_log_file)", &runs, Some((50_000, "entry")));
    println!("    allocs/entry: {:.2}", (allocs().0 - a0) as f64 / (50_000 * REPS) as f64);
    let mut runs = vec![];
    for _ in 0..REPS {
        let ti = Instant::now();
    ...
    stats("tokio Instant::now() (x3 per entry)", &runs, Some((50_000, "call")));

    let _ = std::fs::remove_dir_all(&dir);
}

// ---------------------------------------------------------------- 3. write
async fn measure_write(label: &str, dir: &std::path::Path, t: OpLogType, n: usize, len: usize, reps: usize) {
    let date = Utc::now();
    let texts: Vec<String> = (0..n).map(|i| line(i, len)).collect();
    let mut runs = vec![];
    let mut al = vec![];
    ...
    for r in 0..reps {
        ...
        raw_total = w.definitions["app"].files.values().map(|f| f.queued_bytes + f.logs.len()).sum();
        let a0 = allocs().0;
        let ti = Instant::now();
        ticks = 0;
        loop {
            ...
            ticks += 1;
            if w.definitions["app"].files.values().all(|f| f.logs.is_empty()) {
                break;
            }
        }
        runs.push(ti.elapsed());
    ...
    println!(
        ...
        raw_total as f64 / file_len as f64, al[0] as f64 / ticks as f64
    );
}
...
async fn bench_3_write() {
    ...
    measure_write("typical: 1 x 200 B (one entry per 10 s tick)", &dir.join("t1"), OpLogType::NoSplit, 1, 200, REPS).await;
    measure_write("typical: 50 x 200 B (10 KB per tick)", &dir.join("t50"), OpLogType::NoSplit, 50, 200, REPS).await;
    measure_write("typical: 200 x 1 KB (200 KB per tick)", &dir.join("t200"), OpLogType::NoSplit, 200, 1024, REPS).await;
    measure_write("typical: 500 x 4 KB (2 MB per tick)", &dir.join("t500"), OpLogType::NoSplit, 500, 4096, 3).await;
    measure_write("peak: 10 000 x 1 KB (10 MB queued)", &dir.join("peak"), OpLogType::NoSplit, 10_000, 1024, 3).await;
    measure_write("peak PerDay: 10 000 x 1 KB", &dir.join("peakday"), OpLogType::PerDay, 10_000, 1024, 3).await;
    ...
    let entries: Vec<String> = (0..10_000)
        ...
        .collect();
    for (label, chunk_cap) in [("zlib level 6, 64 000 B chunks, 2 MB cap (as written)", 2 * 1024 * 1024usize), ("zlib level 6, no cap (all 10 MB)", usize::MAX)] {
        let mut runs = vec![];
        let mut out_len = 0;
        ...
        for _ in 0..REPS {
            ...
            loop {
                ...
                while let Some(l) = entries.get(consumed) {
                    ...
                    bytes.push(0x0a);
                    consumed += 1;
                    if bytes.len() > 64000 { break; }
                }
                if bytes.is_empty() { break; }
                enc.write_all(&bytes).unwrap();
                if enc.get_ref().len() > chunk_cap { break; }
            }
    ...
    for lvl in 0u32..=9 {
        ...
        for _ in 0..3 {
            ...
            for l in &entries { enc.write_all(l.as_bytes()).unwrap(); enc.write_all(b"\n").unwrap(); }
            let a = enc.finish().unwrap();
            out_len = a.len();
            std::hint::black_box(a);
            runs.push(ti.elapsed());
        }
        stats(&format!("zlib level {lvl}, all 10 MB (reference only)"), &runs, Some((10_000, "entry")));
        println!("    -> {out_len} B");
    }
    // XOR loop over a 2 MB buffer
    let mut buf = vec![0u8; 2 * 1024 * 1024];
    for (i, b) in buf.iter_mut().enumerate() { *b = (i * 31 % 251) as u8; }
    let mut runs = vec![];
    for _ in 0..REPS {
        let mut a = buf.clone();
        let ti = Instant::now();
    ...
    stats("XOR+checksum loop over 2 MB", &runs, Some((2 * 1024 * 1024, "byte")));
    ...
    io_bench!("tokio create_dir_all (exists)", 200, { tokio::fs::create_dir_all(&iod).await.unwrap() });
    io_bench!("std  create_dir_all (exists)", 200, { std::fs::create_dir_all(&iod).unwrap() });
    io_bench!("tokio metadata", 200, { tokio::fs::metadata(&p).await.unwrap() });
    io_bench!("std  metadata", 200, { std::fs::metadata(&p).unwrap() });
    io_bench!("tokio open(append)", 200, { tokio::fs::File::options().append(true).open(&p).await.unwrap() });
    io_bench!("std  open(append)", 200, { std::fs::File::options().append(true).open(&p).unwrap() });
    io_bench!("tokio open+write_all(8 B)+flush (+drop)", 200, {
        let mut f = tokio::fs::File::options().append(true).open(&p).await.unwrap();
    ...
    std::fs::write(&p, b"OPLog 1.0\n").unwrap();
    io_bench!("tokio spawn_blocking(|| ()) round trip", 1000, { tokio::task::spawn_blocking(|| ()).await.unwrap() });
    io_bench!("tokio cut_interrupted_tail (tiny file, spawn_blocking)", 200, { super::cut_interrupted_tail(&p).await.unwrap() });
    io_bench!("full sequence: create_dir_all+metadata+open+2 writes(8 B, 300 KB)+flush", 100, {
...
fn bench_4_tail_scan() {
    ...
    for (mb, payload) in [(1u64, 10_000usize), (1, 200), (64, 10_000), (64, 200), (64, 100_000)] {
        ...
        stats(&format!("{mb} MB, {payload} B payload/frame ({frames} frames)"), &runs, Some((frames, "frame")));
    }
    let _ = std::fs::remove_dir_all(&dir);
}
...
async fn bench_5_flush_shutdown() {
    ...
    let mut runs = vec![];
    for r in 0..REPS {
        let mut w = worker(&dir.join(format!("e{r}")), OpLogType::NoSplit, HashSet::new(), Duration::from_millis(0));
        w.log("app", Utc::now(), "x");
        w.write_to_files().await;
        let ti = Instant::now();
        w.flush().await;
    ...
    let mut runs = vec![];
    for r in 0..REPS {
        let mut w = worker(&dir.join(format!("n{r}")), OpLogType::NoSplit, HashSet::new(), Duration::from_millis(0));
        let ti = Instant::now();
        w.flush().await;
        runs.push(ti.elapsed());
    }
    ...
    for r in 0..REPS {
        ...
        for name in ["log", "ai", "api"] {
            w.def(OpLogDefinition::new(name, dir.join(format!("i{r}")).to_str().unwrap()).flush_interval(Duration::from_millis(0)));
            w.log(name, Utc::now(), "x");
        }
        w.write_to_files().await;
        let ti = Instant::now();
        for _ in 0..1000 { w.write_to_files().await; }
        runs.push(ti.elapsed());
    }
    stats("idle write tick, 3 defs x 1 file, empty queues", &runs, Some((1000, "tick")));
    // worker flush with 10 000 x 1 KB
    let texts: Vec<String> = (0..10_000).map(|i| line(i, 1024)).collect();
    let mut runs = vec![];
    for r in 0..3 {
        let mut w = worker(&dir.join(format!("f{r}")), OpLogType::NoSplit, HashSet::new(), Duration::from_secs(10));
        for s in &texts { w.log("app", Utc::now(), s); }
        let ti = Instant::now();
        w.flush().await;
        runs.push(ti.elapsed());
    ...
    // flush during an error episode (blocked path): 5 s deadline loop
    {
        ...
        let mut w = worker(&blocker.join("logs"), OpLogType::NoSplit, HashSet::new(), Duration::from_secs(10));
        for s in &texts[..1000] { w.log("app", Utc::now(), s); }
        let a0 = allocs().0;
        let ti = Instant::now();
        w.flush().await;
        let el = ti.elapsed();
        println!("  {:<58} {} ({} allocs; deadline loop, 1000 x 1 KB re-encoded each pass)", "OpLogWorker::flush() during write error episode", fmt(el), allocs().0 - a0);
    }
    // handle shutdown
    let mut runs = vec![];
    for r in 0..3 {
        let op = OpLog::new();
        op.def(OpLogDefinition::new("app", dir.join(format!("s{r}")).to_str().unwrap()).flush_interval(Duration::from_secs(10)));
    ...
    for r in 0..3 {
        let op = OpLog::new();
        op.def(OpLogDefinition::new("app", dir.join(format!("sf{r}")).to_str().unwrap()).flush_interval(Duration::from_secs(10)));
        for s in &texts[..1000] { op.log("app", Utc::now(), s); }
        let ti = Instant::now();
        op.shutdown().await;
        runs.push(ti.elapsed());
    }
    stats("OpLog::shutdown(), 1000 x 1 KB in channel (cap 1024)", &runs, None);
    let _ = std::fs::remove_dir_all(&dir);
}
...
mod fmtv {
    ...
    pub const LOG_PERDAY: &[Item<'static>] = &[
        Item::Numeric(Numeric::Hour, Pad::Zero), Item::Literal(":"),
        Item::Numeric(Numeric::Minute, Pad::Zero), Item::Literal(":"),
        Item::Numeric(Numeric::Second, Pad::Zero),
        Item::Fixed(Fixed::Nanosecond3),
    ];
    pub const PATH_PERDAY: &[Item<'static>] = &[
        Item::Numeric(Numeric::Year, Pad::Zero), Item::Literal("_"),
        Item::Numeric(Numeric::Month, Pad::Zero), Item::Literal("_"),
    ...
    pub fn v1(date: &DateTime<Tz>, text: &str) -> String {
        ...
        date.format("%H:%M:%S.%3f").write_to(&mut s).expect("constant format");
        s.push(' ');
        s.push_str(text);
        s
    }
    pub fn v2(date: &DateTime<Tz>, text: &str) -> String {
        let mut s = String::with_capacity(12 + 1 + text.len());
        date.format_with_items(LOG_PERDAY.iter()).write_to(&mut s).expect("constant format");
        s.push(' ');
        s.push_str(text);
        s
    }
    ...
    pub fn p2(name: &str, date: &DateTime<Tz>) -> String {
        ...
        s.push_str(name);
        s.push('_');
        date.format_with_items(PATH_PERDAY.iter()).write_to(&mut s).expect("constant format");
        s.push_str(".log");
        s
    }
    pub fn items_equal() {
        let parsed: Vec<Item> = StrftimeItems::new("%H:%M:%S.%3f").collect();
        println!("    %H:%M:%S.%3f parses to {parsed:?} (Nanosecond3NoDot is chrono-internal; public Nanosecond3 prints the dot itself)");
        let parsed: Vec<Item> = StrftimeItems::new("%Y_%m_%d").collect();
        assert_eq!(parsed, PATH_PERDAY.to_vec(), "path items differ from strftime parse");
...
fn bench_6_format_variants() {
    ...
    fmtv::items_equal();
    let date = Utc.with_ymd_and_hms(2026, 8, 26, 12, 34, 56).unwrap().with_timezone(&chrono_tz::Europe::Warsaw);
    let texts: Vec<String> = (0..50_000).map(|i| line(i, 100)).collect();
    macro_rules! m {
        ($label:expr, $f:expr) => {{
            let mut runs = vec![];
            ...
            for _ in 0..REPS {
                let ti = Instant::now();
                for s in &texts { std::hint::black_box($f(&date, s)); }
                runs.push(ti.elapsed());
            }
    ...
    m!("v0 current: format!(\"{} {text}\", date.format(F))", fmtv::v0);
    m!("v1w: write!(String::with_capacity, \"{} {text}\", ..)", fmtv::v1w);
    m!("v1: date.format(F).write_to(&mut String::with_capacity)", fmtv::v1);
    m!("v2: v1 + const Item slice (format_with_items)", fmtv::v2);
    m!("p0 current: format!(\"{}_{}.log\", name, date.format(P))", |d, _s| fmtv::p0("app", d));
    m!("p2: push_str + write_to + const items", |d, _s| fmtv::p2("app", d));
    ...
    for h in 0..(4 * 366 * 24) { dates.push(start + chrono::Duration::hours(h) + chrono::Duration::milliseconds(h % 1000)); }
    for m in 0..3600 { dates.push(Utc.with_ymd_and_hms(2026, 3, 29, 0, 0, 0).unwrap() + chrono::Duration::seconds(m * 3) + chrono::Duration::nanoseconds(m * 12345)); }
    for m in 0..3600 { dates.push(Utc.with_ymd_and_hms(2026, 10, 25, 0, 0, 0).unwrap() + chrono::Duration::seconds(m * 3)); }
    ...
    dates.push(Utc.with_ymd_and_hms(-10000, 1, 1, 0, 0, 0).unwrap());
    let samples = ["", "x", "  padded  ", "multi\nline", "ŻAŁOŚĆ ąę", &"y".repeat(5000)];
    let mut n = 0;
    for d in &dates {
...
async fn bench_7_proof_c1() {
    ...
    fn old_entry(t: &OpLogType, no_date: bool, date: &DateTime<chrono_tz::Tz>, text: &str) -> String {
        let f = match t { OpLogType::NoSplit => "", OpLogType::PerHour => "%H:%M:%S.%3f", OpLogType::PerDay => "%H:%M:%S.%3f", OpLogType::PerMonth => "%d %H:%M:%S.%3f" };
        if f.is_empty() || no_date { text.trim().to_string() } else { format!("{} {text}", date.format(f)) }
    }
    fn old_names(t: &OpLogType, subdirs: bool, date: &DateTime<chrono_tz::Tz>, name: &str, path: &str) -> (String, String) {
        let f = match t { OpLogType::NoSplit => "", OpLogType::PerHour => "%Y_%m_%d_%H", OpLogType::PerDay => "%Y_%m_%d", OpLogType::PerMonth => "%Y_%m" };
        let d = date.format(f);
        if f.is_empty() { (format!("{}.log", name), path.to_string()) }
        else if subdirs { (format!("{}.log", name), format!("{}/{}", path, d)) }
        else { (format!("{}_{}.log", name, d), path.to_string()) }
    }
    let mut dates: Vec<DateTime<Utc>> = vec![];
    let start = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    for h in 0..(4 * 366 * 24) { dates.push(start + chrono::Duration::hours(h) + chrono::Duration::milliseconds(h % 1000)); }
    ...
    dates.push(Utc.with_ymd_and_hms(-10000, 1, 1, 0, 0, 0).unwrap());
    let samples = ["", "x", "  padded  ", "multi\nline", "ŻAŁOŚĆ ąę", &"y".repeat(5000)];
    let pairs = [("app", "logs"), ("a/b", ""), ("", "."), ("x", "C:/deep/dir/")];
    let mut n = 0u64;
    for t in [OpLogType::NoSplit, OpLogType::PerHour, OpLogType::PerDay, OpLogType::PerMonth] {
        for (subdirs, no_date) in [(false, false), (true, false), (false, true), (true, true)] {
            let mut opts = HashSet::new();
            if subdirs { opts.insert(OpLogOption::UseSubDirectories); }
            if no_date { opts.insert(OpLogOption::NoAddDateToLog); }
            for (name, path) in pairs {
                let (tx, rx) = tokio::sync::mpsc::channel(1);
                std::mem::forget(tx);
                let mut w = OpLogWorker::new(rx);
                w.def(OpLogDefinition::new(name, path).log_type(t).options(opts.clone()).flush_interval(Duration::from_secs(1)));
                for d in &dates {
    ...
    println!("    {n} cases (dates x types x options x name/path pairs x texts): keys, names, paths and entries identical");
}
...
async fn bench_8_completeness() {
    ...
    for r in 0..REPS {
        ...
        for name in ["log", "ai", "api"] {
            w.def(OpLogDefinition::new(name, d.to_str().unwrap()).flush_interval(Duration::from_millis(0)));
            for s in &texts { w.log(name, Utc::now(), s); }
        }
        let ti = Instant::now();
        w.write_to_files().await;
        runs.push(ti.elapsed());
    ...
    stats("tick: 3 definitions x 200 x 1 KB (3 files, new)", &runs, None);
    let mut runs = vec![];
    for r in 0..REPS {
        let (tx, rx) = tokio::sync::mpsc::channel(32);
        std::mem::forget(tx);
    ...
    stats("tick: 3 definitions x 200 x 1 KB (3 files, existing: tail scan + append)", &runs, None);

    // (b) PerHour rollover: first tick creates hour H file, next tick creates H+1 file; then plain append
    let mut new_file = vec![]; let mut roll = vec![]; let mut append = vec![];
    for r in 0..REPS {
        let mut w = worker(&dir.join(format!("roll{r}")), OpLogType::PerHour, HashSet::new(), Duration::from_millis(0));
        let h0 = Utc.with_ymd_and_hms(2026, 8, 26, 10, 0, 0).unwrap();
        w.log("app", h0, "entry in hour 10");
        let ti = Instant::now(); w.write_to_files().await; new_file.push(ti.elapsed());
        w.log("app", h0 + chrono::Duration::hours(1), "entry in hour 11");
        let ti = Instant::now(); w.write_to_files().await; roll.push(ti.elapsed());
        w.log("app", h0 + chrono::Duration::hours(1), "another entry in hour 11");
        let ti = Instant::now(); w.write_to_files().await; append.push(ti.elapsed());
        assert_eq!(w.definitions["app"].files.len(), 2);
    }
    stats("PerHour: tick creating the first file (create+magic+header)", &new_file, None);
    stats("PerHour: rollover tick (new file for H+1, old file idle)", &roll, None);
    stats("PerHour: append tick (2 files in map, 1 with entries)", &append, None);

    // (c) failing tick (path blocked by a file) — the per-500 ms cost of an error episode
    {
        let blocker = dir.join("blocker");
        std::fs::write(&blocker, b"file").unwrap();
        let mut w = worker(&blocker.join("logs"), OpLogType::NoSplit, HashSet::new(), Duration::from_millis(0));
        for s in &texts { w.log("app", Utc::now(), s); }
        let mut runs = vec![];
        for _ in 0..REPS {
            let ti = Instant::now();
            for _ in 0..100 { w.write_to_files().await; }
            runs.push(ti.elapsed());
        }
        stats("failing tick (path error, fails before encoding), 200 x 1 KB queued", &runs, Some((100, "tick")));
    }
    // (d) first tick after an error episode on a 64 MB NoSplit file: tail scan + append
    {
        ...
        let mut runs = vec![];
        for _ in 0..REPS {
            let mut w = worker(&d, OpLogType::NoSplit, HashSet::new(), Duration::from_millis(0));
        ...
        stats(&format!("first tick on a 64 MB file ({frames} frames): tail scan + append"), &runs, None);
    }
    // (e) get_info on the handle (empty queue) and log_bundle producer cost
    let op = OpLog::new();
    op.def(OpLogDefinition::new("app", dir.join("info").to_str().unwrap()).flush_interval(Duration::from_secs(10)));
    let mut runs = vec![];
    for _ in 0..REPS {
        let ti = Instant::now();
        for _ in 0..100 { op.get_info().await.unwrap(); }
        runs.push(ti.elapsed());
    }
    stats("OpLog::get_info() round trip, empty queue", &runs, Some((100, "call")));
    let mut runs = vec![]; let mut al = vec![];
    for _ in 0..REPS {
        tokio::time::sleep(Duration::from_millis(50)).await;
        let a0 = allocs().0;
        let ti = Instant::now();
        let mut b = crate::messages::OpLogBundle::new();
        for s in &texts { b = b.add_log(crate::messages::OpLogData { log_name: "app".to_string(), log: s.clone(), date: Utc::now() }); }
        op.log_bundle(b);
        runs.push(ti.elapsed());
        al.push(allocs().0 - a0);
    }
    stats("OpLog::log_bundle() with 200 x 1 KB (producer side)", &runs, Some((200, "entry")));
    ...
    for burst in [1_000usize, 10_000, 50_000] {
        ...
        let ti = Instant::now();
        for _ in 0..burst { op.log("app", Utc::now(), &text); }
        let el = ti.elapsed();
        tokio::time::sleep(Duration::from_millis(300)).await;
        let info = op.get_info().await.unwrap();
        println!("  burst {burst:>6} x 100 B in {:>9}: reached the worker {:>6} ({:.1}%), dropped at the channel {:>6}",
            fmt(el), info.number_of_logs, info.number_of_logs as f64 / burst as f64 * 100.0, burst.saturating_sub(info.number_of_logs));
    }
    op.shutdown().await;
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn bench_9_expect_unreachable() {
    ...
    for f in ["", "%H:%M:%S.%3f", "%d %H:%M:%S.%3f", "%Y_%m_%d_%H", "%Y_%m_%d", "%Y_%m"] {
        let items: Vec<Item> = StrftimeItems::new(f).collect();
        for it in &items {
            match it {
                Item::Literal(_) | Item::Space(_) => {}
                Item::Numeric(n, _) => assert!(matches!(n, Numeric::Hour | Numeric::Minute | Numeric::Second | Numeric::Day | Numeric::Month | Numeric::Year), "{f}: {it:?}"),
                Item::Fixed(Fixed::Internal(_)) => {} // Nanosecond3NoDot: needs `time` only
                other => panic!("{f}: unexpected item {other:?}"),
            }
        }
        assert!(!items.contains(&Item::Error), "{f} parses without an error item");
        println!("    {f:<18} -> {} items, no Item::Error, only date/time numerics + literals", items.len());
    }
}
